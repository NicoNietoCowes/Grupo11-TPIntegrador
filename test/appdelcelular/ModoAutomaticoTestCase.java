...
class ModoAutomaticoTestCase {
	...
	private ModoAutomatico modoAutomatico = new ModoAutomatico(appRef);
	...
	void testPosibleInicioDeEstacionamiento() {
		...
		verify(appRef).inicioDeEstacionamiento(modoAutomatico.getReloj());
	}
	...
	void testPosibleFinDeEstacionamiento() {
		...
		verify(appRef).finDeEstacionamiento(modoAutomatico.getReloj());
	}
